...
int main(int argc, char* argv[])
{
  ...
  const std::string genDigFile{"ft0digits.root"};
  const std::string decDigFile{"o2_ft0digits.root"};
  ...
  tree->SetBranchAddress(branchBC.c_str(), &ft0BCDataPtr);
  tree->SetBranchAddress(branchCH.c_str(), &ft0ChDataPtr);
  ...
  tree2->SetBranchAddress(branchBC.c_str(), &ft0BCDataPtr2);
  tree2->SetBranchAddress(branchCH.c_str(), &ft0ChDataPtr2);

  // Store the data from the files in these
  std::vector<o2::ft0::Digit> digits;
  // std::vector<std::vector<o2::ft0::ChannelData>> channels;
  std::vector<std::unordered_map<uint8_t, o2::ft0::ChannelData>> channels;

  std::vector<o2::ft0::Digit> digits2;
  // std::vector<std::vector<o2::ft0::ChannelData>> channels2;
  std::vector<std::unordered_map<uint8_t, o2::ft0::ChannelData>> channels2;

  int nCh = 0;
  for (int iEntry = 0; iEntry < tree->GetEntries(); iEntry++) {
    tree->GetEntry(iEntry);
    for (int iBC = 0; iBC < digitsBC.size(); iBC++) {
      o2::ft0::Digit& digit = digitsBC[iBC];
      digits.push_back(digit);
      gsl::span<const o2::ft0::ChannelData> bcChannels = digit.getBunchChannelData(digitsCh);
      // channels.push_back(move(std::vector<o2::ft0::ChannelData>(bcChannels.begin(), bcChannels.end())));
      channels.push_back(std::unordered_map<uint8_t, o2::ft0::ChannelData>());
  ...
  for (int iEntry = 0; iEntry < tree2->GetEntries(); iEntry++) {
    ...
    for (int iBC = 0; iBC < digitsBC2.size(); iBC++) {
      o2::ft0::Digit& digit = digitsBC2[iBC];
      digits2.push_back(digit);
      gsl::span<const o2::ft0::ChannelData> bcChannels = digit.getBunchChannelData(digitsCh2);
      // channels2.push_back(move(std::vector<o2::ft0::ChannelData>(bcChannels.begin(), bcChannels.end())));
      channels2.push_back(std::unordered_map<uint8_t, o2::ft0::ChannelData>());
      for (int iCh = 0; iCh < bcChannels.size(); iCh++) {
  ...
  for (int iBC = 0; iBC < nDigitsCompared; iBC++) {
    ...
    // AM: Digit::operator== doesn't check mEventStatus, should it?
    ...
    if (digit.ref.getEntries() != digit2.ref.getEntries()
        ...
        || digit.getEventStatusWord() != digit2.getEventStatusWord()) {
      nDigitsDiffs++;
