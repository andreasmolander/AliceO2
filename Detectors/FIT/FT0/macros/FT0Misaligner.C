...
void FT0Misaligner(const std::string& ccdbHost = "http://ccdb-test.cern.ch:8080", long tmin = 0, long tmax = -1,
                   ...
                   const std::string& comment = "Alignment object for FT0")
{
  ...
  if (exactAlignment) {
    pars = generateExactMisalignment(xA, yA, zA, psiA, thetaA, phiA);
  } else {
  ...
  if (exactAlignment) {
    pars = generateExactMisalignment(xC, yC, zC, psiC, thetaC, phiC);
  } else {
  ...
  // AM TODO: align detector cells as well!
