...
int main(int argc, char* argv[])
{
  ...
  const std::string genDigFile{"fdddigits.root"};
  const std::string decDigFile{"o2_fdddigits.root"};
  const std::string branchBC{"FDDDigit"};
  const std::string branchCH{"FDDDigitCh"};
  ...
  tree->SetBranchAddress(branchBC.c_str(), &fddBCDataPtr);
  tree->SetBranchAddress(branchCH.c_str(), &fddChDataPtr);
  ...
  tree2->SetBranchAddress(branchBC.c_str(), &fddBCDataPtr2);
  tree2->SetBranchAddress(branchCH.c_str(), &fddChDataPtr2);

  // Store the data from the files in these
  std::vector<o2::fdd::Digit> digits;
  // std::vector<std::vector<o2::fdd::ChannelData>> channels;
  std::vector<std::unordered_map<uint8_t, o2::fdd::ChannelData>> channels;

  std::vector<o2::fdd::Digit> digits2;
  // std::vector<std::vector<o2::fdd::ChannelData>> channels2;
  std::vector<std::unordered_map<uint8_t, o2::fdd::ChannelData>> channels2;

  int nCh = 0;
  for (int iEntry = 0; iEntry < tree->GetEntries(); iEntry++) {
    tree->GetEntry(iEntry);
    for (int iBC = 0; iBC < digitsBC.size(); iBC++) {
      o2::fdd::Digit& digit = digitsBC[iBC];
      digits.push_back(digit);
      gsl::span<const o2::fdd::ChannelData> bcChannels = digit.getBunchChannelData(digitsCh);
      // channels.push_back(move(std::vector<o2::fdd::ChannelData>(bcChannels.begin(), bcChannels.end())));
      channels.push_back(std::unordered_map<uint8_t, o2::fdd::ChannelData>());
      for (int iCh = 0; iCh < bcChannels.size(); iCh++) {
        channels.back()[bcChannels[iCh].getChannelID()] = std::move(bcChannels[iCh]);
      }
      nCh += bcChannels.size();
    }
  }
  ...
  for (int iEntry = 0; iEntry < tree2->GetEntries(); iEntry++) {
    ...
    for (int iBC = 0; iBC < digitsBC2.size(); iBC++) {
      o2::fdd::Digit& digit = digitsBC2[iBC];
      digits2.push_back(digit);
      gsl::span<const o2::fdd::ChannelData> bcChannels = digit.getBunchChannelData(digitsCh2);
      // channels2.push_back(move(std::vector<o2::fdd::ChannelData>(bcChannels.begin(), bcChannels.end())));
      channels2.push_back(std::unordered_map<uint8_t, o2::fdd::ChannelData>());
      for (int iCh = 0; iCh < bcChannels.size(); iCh++) {
        channels2.back()[bcChannels[iCh].getChannelID()] = std::move(bcChannels[iCh]);
      }
  ...
  for (int iBC = 0; iBC < nDigitsCompared; iBC++) {
    ...
    for (auto& iCh : nChannels <= nChannels2 ? channels[iBC] : channels2[iBC]) {
      ...
      // AM: ChannelData::operator== doesn't check mFEEBits, should it?
      if (!(ch == ch2) || (ch.mFEEBits != ch2.mFEEBits)) { // ChannelData::operator!= not implemented
        nChDiffs++;
